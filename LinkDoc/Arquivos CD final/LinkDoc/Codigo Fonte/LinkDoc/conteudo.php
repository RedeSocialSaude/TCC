...
<div id="page-wrapper">
    ...
    <?php
        $idAreaInf = $_GET['action'];

            $con = mysqli_connect($host, $user, $password, $dbname)
                        or die('Could not connect to the database server' . mysqli_connect_error());
                $sql = "SELECT titulo, conteudo, caminho_imagem_informativa FROM mydb.area_informativa where id_area_informativa = '$idAreaInf'";
                if ($resultado = mysqli_query($con, $sql)) {
                    while ($row = mysqli_fetch_object($resultado)) {
                        $Titulo = $row->titulo;
                        $texto = $row->conteudo;
                        $caminhoImg = $row->caminho_imagem_informativa;
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">
                    <?php echo $Titulo; ?> <small>Especificações da área:</small>
                </h1>
                <ol class="breadcrumb">
                    <li class="active">
                        <i class="fa fa-dashboard"></i> Área Informativa
                    </li>
                    <li class="active">
                        <i class="fa fa-bookmark-o"></i> <?php echo $Titulo; ?>
                    </li>
                </ol>
            </div>
        </div>
        ...
        <h4 style="color: black;"> <a href="#" id="myListM">Lista de médicos</a></h4>

        <hr>

        <div>
                <p><img src="imagens\<?php echo $caminhoImg; ?>" width="350px" height="350px" style="position:relative; left:28%;"></p></div>
               
        </div>

        <p><div id="txt"><p><?php echo $texto; ?></pre></div> </p>  
...
<div class="modal fade" id="myModal2" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Lista de <?php echo $Titulo."s"; ?></h4>
            </div>

            <?php
                $con = mysqli_connect($host, $user, $password, $dbname)
        or die('Could not connect to the database server' . mysqli_connect_error());
$sql = "SELECT pessoa_id_pessoa, local_trabalho FROM mydb.profissional_saude where area_informativa_id_area_informativa = '$idAreaInf'";
if ($resultado = mysqli_query($con, $sql)) {
    while ($row = mysqli_fetch_object($resultado)) {
        $IdProf[] = $row->pessoa_id_pessoa;
        $LocalProf[] = $row->local_trabalho;
    }
}
...
$con = mysqli_connect($host, $user, $password, $dbname)
        ...
        for ($i=0; $i < count($IdProf); $i++) { 
            $sql = "SELECT nome_pessoa, sobrenome_pessoa, num_pessoa, cidade, caminho_imagem FROM mydb.pessoa where id_pessoa = '$IdProf[$i]';";
if ($resultado = mysqli_query($con, $sql)) {
    while ($row = mysqli_fetch_object($resultado)) {
        $NomeProf[] = $row->nome_pessoa;
        $SobrenomeProf[] = $row->sobrenome_pessoa;
        $CidadeProf[] = $row->cidade;
        $ImagemProf[] = $row->caminho_imagem;
        $TelProf[] = $row->num_pessoa;
    }
}
}
...
<div class="modal-body">
                <div class="form-group">                    
                        ...
                        <div class="panel-body">
                            <br>
                            <p style="position:relative; left:45%">Lista vazia</p>
...
for ($i=0; $i < count($IdProf); $i++) { 
            ...
            <div class="modal-body">
                <div class="form-group">                    
                        ...
                        <div class="panel-body">
                        <span class="pull-left">
                                        <img class="img-circle" src="imagens\<?php echo $ImagemProf[$i]; ?>" alt="" style="position:relative; right:14px;">
                                    </span>
                            <div class="list-group">
                                <li class="media m-b-md">
                                    <div class="list-group-item">
                                        <div>
                                            <p><?php echo $NomeProf[$i]." ".$SobrenomeProf[$i]; ?></p>
                                            <span class="help-block"><?php echo $CidadeProf[$i]." - ".$LocalProf[$i]; ?></span>
                                            <span class="help-block"><?php echo "Telefone: ".$TelProf[$i]; ?></span>
                                        </div>
                                    </div>
                                </li>
                            </div>
